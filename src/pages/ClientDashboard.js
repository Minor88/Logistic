...
function ManagerDashboard() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      //const currentClientId = localStorage.getItem('user_id'); // Получаем ID клиента из localStorage
      const headers = { Authorization: `Token ${token}` };

      //const requestsResponse = await axios.get(`${API_BASE_URL}/logistic/api/requests/?client=${currentClientId}`, { headers });
  ...
  const showRequestModal = () => {
    console.log('Текущий user_id перед открытием модального окна:', localStorage.getItem('user_id'));
    ...
    >
      ...
      {/*<div className="form-field">
        ...
        <Input
          ...
          onChange={(e) => setNewRequest({ ...newRequest, actual_weight: e.target.value })}
        />
      ...
      {/*<div className="form-field">
        ...
        <Input
          ...
          onChange={(e) => setNewRequest({ ...newRequest, actual_volume: e.target.value })}
        />
...
const fileModal = (
  ...
  const handleAddRequest = async () => {
    const token = localStorage.getItem('token');
    const currentAuthUserId = localStorage.getItem('user_id'); // Получаем ID клиента из localStorage
    const headers = { Authorization: `Token ${token}` };
    ...
    try {
      ...
      // Находим профиль, соответствующий текущему auth_user_id
      ...
      const preparedRequest = {
        ...newRequest,
        client: userProfile.id, // Используем ID профиля UserProfile, а не auth_user
        status: 'new',                      // Устанавливаем статус "Новая заявка"
      ...
      if (createdRequest && createdRequest.id) {
        if (selectedFiles.length > 0) {
          console.log('Загрузка файлов для заявки с ID:', createdRequest.id);  // Проверяем ID заявки перед загрузкой
      ...
      setNewRequest({ description: '', declared_weight: '', declared_volume: null, comment: '', client: '', status: '', shipment: '' });
      setSelectedFiles([]);  // Очищаем выбранные файлы
      message.success('Заявка успешно добавлена');
    } catch (error) {
      console.error('Ошибка добавления заявки:', error.response?.data || error.message);
  ...
  const columnsRequests = [
    ...
    { title: 'Фактический объем (м³)', dataIndex: 'actual_volume', key: 'actual_volume', editable: false },
    { title: 'Комментарий', dataIndex: 'comment', key: 'comment', editable: true,
